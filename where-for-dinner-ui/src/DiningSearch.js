...
function DiningSearch(props)
{
    ...
    const deleteSearch = (e)=>
    {
       if (window.confirm("Do you wish do delete the search " + props.search.name))
       {
         ...
         //axios.delete('http://where-for-dinner.workloads.tanzu-lab.com/api/search/' + props.search.id)
         .then(rest =>{
    ...
    const loadSearchAvailability= () =>
    {
      ...
      //axios.get('http://where-for-dinner.workloads.tanzu-lab.com/api/availability/' + props.search.name)
      .then(resp => 
      {
